...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-4">
        <form method="GET" class="flex items-center space-x-4">
            <input type="hidden" name="c" value="ambiente">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-primary to-primary-hover text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Sede</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($pagination['data'])): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg">
                                    <?php echo !empty($busqueda) ? 'No se encontraron resultados' : 'No hay ambientes registrados'; ?>
                                </p>
                                <?php if (!empty($busqueda)): ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($pagination['data'] as $ambiente): 
                            $sede = Sede::find($ambiente->getSedeId());
                            $sedeNombre = $sede ? $sede->getNombre() : 'N/A';
                        ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <?php echo htmlspecialchars($ambiente->getId()); ?>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <a href="<?php echo url('ambiente', 'updateshow', ['id' => $ambiente->getId()]); ?>" 
                                   class="text-primary hover:text-primary-dark mx-2" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" 
                                   onclick="showDeleteModal('<?php echo url('ambiente', 'delete', ['id' => $ambiente->getId()]); ?>', '¿Está seguro de eliminar el ambiente <?php echo htmlspecialchars($ambiente->getNombre()); ?>?')"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        <?php if ($pagination['page'] > 1): ?>
                            <a href="<?php echo url('ambiente', 'show', ['page' => $pagination['page'] - 1, 'search' => $busqueda]); ?>" 
                        ...
                        ?>
                            <a href="<?php echo url('ambiente', 'show', ['page' => $i, 'search' => $busqueda]); ?>" 
