...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-gray-800">
                ...
                Gestión de Títulos de Programa
            </h2>
            <a href="index.php?c=tituloPrograma&a=register&role=<?php echo $userRole; ?>" 
               class="bg-primary hover:bg-primary-hover text-white px-6 py-3 rounded-lg transition">
                <i class="fas fa-plus mr-2"></i>Nuevo Título
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4">
        <form method="GET" class="flex items-center space-x-4">
            <input type="hidden" name="c" value="tituloPrograma">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($pagination['data'])): ?>
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg">
                                    <?php echo !empty($busqueda) ? 'No se encontraron resultados' : 'No hay títulos registrados'; ?>
                                </p>
                                <?php if (!empty($busqueda)): ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($pagination['data'] as $titulo): ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <?php echo htmlspecialchars($titulo->getId()); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars($titulo->getNombre()); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <a href="index.php?c=tituloPrograma&a=updateshow&id=<?php echo $titulo->getId(); ?>&role=<?php echo $userRole; ?>" 
                                   class="text-primary hover:text-primary-dark mx-2" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" 
                                   onclick="showDeleteModal('index.php?c=tituloPrograma&a=delete&id=<?php echo $titulo->getId(); ?>&role=<?php echo $userRole; ?>', '¿Está seguro de eliminar el título <?php echo htmlspecialchars($titulo->getNombre()); ?>?')"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        <?php if ($pagination['current_page'] > 1): ?>
                            <a href="?c=tituloPrograma&a=show&role=<?php echo $userRole; ?>&page=<?php echo $pagination['current_page'] - 1; ?>&search=<?php echo urlencode($busqueda); ?>" 
                        ...
                        ?>
                            <a href="?c=tituloPrograma&a=show&role=<?php echo $userRole; ?>&page=<?php echo $i; ?>&search=<?php echo urlencode($busqueda); ?>" 
