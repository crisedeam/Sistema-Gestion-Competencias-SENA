...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex justify-between items-center">
            ...
            <a href="<?php echo url('sede', 'register'); ?>" 
    ...
    <div class="bg-white rounded-xl shadow-md p-4">
        <form method="GET" class="flex items-center space-x-4">
            <input type="hidden" name="c" value="sede">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($pagination['data'])): ?>
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg">
                                    <?php echo !empty($busqueda) ? 'No se encontraron resultados' : 'No hay sedes registradas'; ?>
                                </p>
                                <?php if (!empty($busqueda)): ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($pagination['data'] as $sede): ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <?php echo htmlspecialchars($sede->getId()); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars($sede->getNombre()); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <a href="<?php echo url('sede', 'updateshow', ['id' => $sede->getId()]); ?>" 
                                   class="text-primary hover:text-primary-dark mx-2" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" 
                                   onclick="showDeleteModal('<?php echo url('sede', 'delete', ['id' => $sede->getId()]); ?>', '¿Está seguro de eliminar la sede <?php echo htmlspecialchars($sede->getNombre()); ?>?')"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        <?php if ($pagination['page'] > 1): ?>
                            <a href="<?php echo url('sede', 'show', ['page' => $pagination['page'] - 1, 'search' => $busqueda]); ?>" 
                        ...
                        ?>
                            <a href="<?php echo url('sede', 'show', ['page' => $i, 'search' => $busqueda]); ?>" 
