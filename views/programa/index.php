...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex justify-between items-center">
            ...
            <a href="<?php echo url('programa', 'register'); ?>" 
    ...
    <div class="bg-white rounded-xl shadow-md p-4">
        <form method="GET" class="flex items-center space-x-4">
            <input type="hidden" name="controller" value="programa">
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($busqueda); ?>" 
                           placeholder="Buscar por código, denominación o tipo..." 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-primary to-primary-hover text-white">
                    <tr>
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Código</th>
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Denominación</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($pagination['data'])): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg">
                                    <?php echo !empty($busqueda) ? 'No se encontraron resultados' : 'No hay programas registrados'; ?>
                                </p>
                                <?php if (!empty($busqueda)): ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($pagination['data'] as $programa): ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <?php echo htmlspecialchars($programa->getCodigo()); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars($programa->getDenominacion()); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars($programa->getTipo()); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <a href="<?php echo url('programa', 'updateshow', ['id' => $programa->getCodigo()]); ?>" 
                                   class="text-primary hover:text-primary-dark mx-2" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" 
                                   onclick="showDeleteModal('<?php echo url('programa', 'delete', ['id' => $programa->getCodigo()]); ?>', '¿Está seguro de eliminar el programa <?php echo htmlspecialchars($programa->getDenominacion()); ?>?')"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        <?php if ($pagination['current_page'] > 1): ?>
                            <a href="<?php echo url('programa', 'show', ['page' => $pagination['current_page'] - 1, 'search' => $busqueda]); ?>" 
                        ...
                        ?>
                            <a href="<?php echo url('programa', 'show', ['page' => $i, 'search' => $busqueda]); ?>" 
