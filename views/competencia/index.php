...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <div class="flex justify-between items-center">
            ...
            <a href="<?php echo url('competencia', 'register'); ?>" 
               class="bg-primary hover:bg-primary-hover text-white px-6 py-3 rounded-lg transition">
                <i class="fas fa-plus mr-2"></i>Nueva Competencia
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-4">
        <form method="GET" class="flex items-center space-x-4">
            <input type="hidden" name="controller" value="competencia">
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($busqueda); ?>" 
                           placeholder="Buscar por ID, nombre corto, unidad de competencia o horas..." 
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-primary to-primary-hover text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Nombre Corto</th>
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Unidad de Competencia</th>
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Horas</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($pagination['data'])): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                ...
                                <p class="text-lg">
                                    <?php echo !empty($busqueda) ? 'No se encontraron resultados' : 'No hay competencias registradas'; ?>
                                </p>
                                <?php if (!empty($busqueda)): ?>
                    ...
                    <?php else: ?>
                        <?php foreach ($pagination['data'] as $competencia): ?>
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <?php echo htmlspecialchars($competencia->getId()); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars($competencia->getNombreCorto()); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars($competencia->getNombreUnidad()); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold">
                                    <?php echo $competencia->getHoras(); ?> hrs
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <a href="<?php echo url('competencia', 'updateshow', ['id' => $competencia->getId()]); ?>" 
                                   class="text-primary hover:text-primary-dark mx-2" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" 
                                   onclick="showDeleteModal('<?php echo url('competencia', 'delete', ['id' => $competencia->getId()]); ?>', '¿Está seguro de eliminar la competencia <?php echo htmlspecialchars($competencia->getNombreCorto()); ?>? Esta acción verificará si tiene dependencias.')"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        <?php if ($pagination['current_page'] > 1): ?>
                            <a href="<?php echo url('competencia', 'show', ['page' => $pagination['current_page'] - 1, 'search' => $busqueda]); ?>" 
                        ...
                        ?>
                            <a href="<?php echo url('competencia', 'show', ['page' => $i, 'search' => $busqueda]); ?>" 
